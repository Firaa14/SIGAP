...
<div class="filter-bar" id="equipment-filter-bar">
    <div class="filter-group">
        ...
        <input type="text"
               ...
               id="search-equipment"
               placeholder="Cari equipment, KKS, ASSETNUM..."
               autocomplete="off">
    </div>
        ...
        <table class="data-table" id="equipment-table">
            ...
            <tbody id="equipment-tbody">
                ...
                <tr class="equipment-row"
                    data-unit="{{ $eq['unit'] }}"
                    data-status="{{ $eq['status_operasi'] }}"
                    data-search="{{ strtolower($eq['equipment'] . ' ' . $eq['kks'] . ' ' . $eq['assetnum'] . ' ' . $eq['unit'] . ' ' . $eq['system']) }}">
                    <td class="col-no">{{ $index + 1 }}</td>
                    <td>
                        <span style="font-size:11.5px; font-weight:500; color:var(--text-secondary);">{{ $eq['unit'] }}</span>
                    </td>
                    <td>
...
<div style="display:flex; justify-content:space-between; align-items:center; margin-top:16px; flex-wrap:wrap; gap:8px;">
    ...
    <div style="display:flex; gap:8px;">
        @php
            $pltaSlugs = ['sengguruh','sutami','wlingi','lodoyo','tulungagung','mendalan','siman','wonorejo','plengan','lamajan','cikalong','bengkok','dago'];
